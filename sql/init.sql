...
-- ============================================================
-- Agent Memories — persistent cross-session memory
-- ============================================================
...
-- Dedup: identical content within the same scope is a no-op (importance bump instead)
CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_memories_dedup
    ON agent_memories (COALESCE(project_id, ''), md5(content));
-- Keyword retrieval upgrade path — lets recall_memories() add a query= filter
-- later with no migration; not used for ranking in v1.
