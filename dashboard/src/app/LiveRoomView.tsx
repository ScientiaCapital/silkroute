...
// "replay" (scripted, the default) vs "live agent run" (a real run_agent() loop
// over local Ollama) — a different axis from LiveBadge, which means "API reachable".
function RunModeBadge({ liveRun }: { liveRun: boolean }) {
  return (
    <span
      className={
        "text-[10px] uppercase tracking-wide px-2 py-0.5 rounded-full " +
        (liveRun ? "text-amber-300 bg-amber-400/10" : "text-neutral-500 bg-neutral-800")
      }
      title={liveRun ? "A real agent loop ran against the mock room" : "Scripted replay"}
    >
      {liveRun ? "live agent run" : "replay"}
    </span>
  );
}
    ...
    <div className="flex flex-col gap-4">
      {trace.map((ev, i) => {
        ...
        if (ev.type === "tool_call") {
          const name = str(ev.data, "tool_name");
          return (
            <p key={i} className="font-mono text-xs text-amber-400/80 pl-1">
              {name ? <>→ MCP call: {name}()</> : <>→ {String(ev.data["count"] ?? "")} tool call(s)</>}
...
export default function LiveRoomView() {
  ...
  // Shared across the auto-played replay and the "Run live agent" button so
  // only one EventSource's frames ever land in `trace` at a time.
  const traceEsRef = useRef<EventSource | null>(null);

  const openTraceStream = (url: string) => {
    traceEsRef.current?.close();
    const es = new EventSource(url);
    traceEsRef.current = es;
    es.onmessage = (event) => {
      if (traceEsRef.current !== es) return; // superseded by a newer stream
      if (event.data.startsWith("[")) {
        // [DONE] or [ERROR ...] — terminal frame
        es.close();
        setLiveRunning(false);
        const match = /^\[ERROR\]\s*(.*)/.exec(event.data);
        if (match?.[1]) setLiveError(match[1]);
        return;
      }
      try {
    ...
    es.onerror = () => {
      if (traceEsRef.current === es) {
        es.close();
        setLiveRunning(false);
        setLiveError((prev) => prev ?? "Connection lost — the stream ended unexpectedly.");
      }
    };
    return es;
  };
  ...
  const runLiveAgent = () => {
    ...
    setLiveRun(true);
    setLiveRunning(true);
    openTraceStream(`${API_BASE}/demo/stream?live=true`);
  };
  ...
  useEffect(() => {
    ...
    openTraceStream(`${API_BASE}/demo/stream`);
        ...
        <div className="sr-fade-up bg-neutral-900 border border-neutral-800 rounded-xl p-6" style={{ animationDelay: "480ms" }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <RunModeBadge liveRun={liveRun} />
              <LiveBadge live={live} />
            </div>
          </div>
          {trace.length > 0 ? <TraceView trace={trace} /> : <StaticTrace />}
          {liveError && (
            <p className="mt-3 text-xs text-red-400 bg-red-400/5 rounded px-3 py-2">
              ⚠ {liveError}
            </p>
          )}
          <button
            ...
            className="mt-4 bg-neutral-800 hover:bg-neutral-700 disabled:opacity-50 text-neutral-200 font-medium text-xs px-3 py-1.5 rounded-lg transition-colors"
          >
            {liveRunning ? "Running live agent…" : "Run live agent"}
          </button>
        </div>
